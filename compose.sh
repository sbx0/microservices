...
up() {
  if [ -n "$1" ]; then
    docker-compose -f docker-compose-prod.yml --env-file .env --compatibility up -d "$1"
  else
    docker-compose -f docker-compose-prod.yml --env-file .env --compatibility up -d microservices-gateway
  fi
}
